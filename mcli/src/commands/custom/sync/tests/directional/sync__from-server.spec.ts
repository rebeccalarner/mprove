...
test('--from-server downloads server changes and overwrites local changes', async () => {
  let testId = 'mcli_sync__directional__from_server';
  let isPass = false;
  ...
  await retry(async () => {
    ...
    await fse.writeFile(`${repoPath}/.env`, 'preserve me');
    ...
    assert.equal(await fse.readFile(`${repoPath}/.env`, 'utf8'), 'preserve me');
